...
> Mirror of [`CLAUDE.md`](CLAUDE.md) for non-Claude coding agents (Cursor,
> Codex, Aider, etc.). Both files must stay in sync. If you edit one, edit the
> other.
...
- **Confirm before non-trivial actions** — file batches, dependency installs,
  destructive commands, architectural decisions.
...
```
veda-ai/  (root)
├── CLAUDE.md                   ← Claude's rulebook
├── AGENTS.md                   ← this file — mirror for non-Claude agents
